...
public class RichDrawableHelper implements DrawableEnriched {
    ...
    @ColorInt
    private final int mDrawableTint;

    public RichDrawableHelper(@NonNull Context context, AttributeSet attrs, int defStyleAttr,
                              int defStyleRes) {
        mContext = context;

        TypedArray array = context.obtainStyledAttributes(attrs, R.styleable.TextViewRichDrawable,
                defStyleAttr, defStyleRes);

        try {
            mDrawableWidth = array.getDimensionPixelSize(
                    R.styleable.TextViewRichDrawable_compoundDrawableWidth, UNDEFINED);
            mDrawableHeight = array.getDimensionPixelSize(
                    R.styleable.TextViewRichDrawable_compoundDrawableHeight, UNDEFINED);
            mDrawableStartVectorId = array.getResourceId(
                    R.styleable.TextViewRichDrawable_drawableStartVector, UNDEFINED);
            mDrawableTopVectorId = array.getResourceId(
                    R.styleable.TextViewRichDrawable_drawableTopVector, UNDEFINED);
            mDrawableEndVectorId = array.getResourceId(
                    R.styleable.TextViewRichDrawable_drawableEndVector, UNDEFINED);
            mDrawableBottomVectorId = array.getResourceId(
                    R.styleable.TextViewRichDrawable_drawableBottomVector, UNDEFINED);
            mDrawableTint = array.getColor(R.styleable.TextViewRichDrawable_drawableTintRich,
                    UNDEFINED);
        } finally {
            array.recycle();
        }
    ...
    public void apply(TextView textView) {
        if (mDrawableWidth > 0 || mDrawableHeight > 0 || mDrawableStartVectorId > 0 ||
                mDrawableTopVectorId > 0 || mDrawableEndVectorId > 0 ||
                mDrawableBottomVectorId > 0) {
            initCompoundDrawables(textView, mDrawableStartVectorId, mDrawableTopVectorId,
                    mDrawableEndVectorId, mDrawableBottomVectorId);
        }
    }

    private void initCompoundDrawables(TextView textView, int drawableStartVectorId,
    ...
    private void scale(Drawable[] drawables) {
        if (mDrawableHeight > 0 || mDrawableWidth > 0) {
            for (Drawable drawable : drawables) {
                ...
                Rect realBounds = new Rect(0, 0, drawable.getIntrinsicWidth(),
