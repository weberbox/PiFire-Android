...
data class Setting<T>(
    ...
    companion object {
        ...
        val wledUseProfiles = Setting("use_profiles", true)
        val wledUseSuggestedProfiles = Setting("use_suggested_presets", false)
        val wledCookingColor = Setting("cooking_color", "blue")
        val wledIdleBrightness = Setting("idle_brightness", 20)
        val wledLedCount = Setting("led_count", 6)
        val wledNightMode = Setting("night_mode", false)
        val wledProfileBooting = Setting("booting", 201)
        val wledProfileCooking = Setting("cooking", 203)
        val wledProfileCooldown = Setting("cooldown", 204)
        val wledProfileFault = Setting("error_fault", 210)
        val wledProfileIdle = Setting("idle", 200)
        val wledProfilePellets = Setting("low_pellets", 208)
        val wledProfileNight = Setting("night_mode", 211)
        val wledProfileOvershoot = Setting("overshoot_alarm", 206)
        val wledProfilePreheat = Setting("preheat", 202)
        val wledProfileProbe = Setting("probe_alarm", 207)
        val wledProfileTarget = Setting("target_reached", 205)
        val wledProfileTimer = Setting("timer_done", 209)
    }
}
