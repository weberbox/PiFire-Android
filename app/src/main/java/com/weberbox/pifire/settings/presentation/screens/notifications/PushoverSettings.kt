...
@Composable
fun PushoverSettingsDestination(
...
@Composable
private fun PushoverSettingsContent(
    state: NotifContract.State,
    onEventSent: (event: NotifContract.Event) -> Unit,
    contentPadding: PaddingValues
) {
    val apiKeySheet = rememberCustomModalBottomSheetState()
    val userKeysSheet = rememberCustomModalBottomSheetState()
    ...
    Column(
        ...
        Preference(
            title = { Text(text = stringResource(R.string.settings_pushover_api)) },
            summary = { Text(text = getSummary(state.serverData.settings.pushoverApiKey)) },
            onClick = { apiKeySheet.open() }
        )
        Preference(
            title = { Text(text = stringResource(R.string.settings_pushover_keys)) },
            summary = { Text(text = getSummary(state.serverData.settings.pushoverUserKeys)) },
            onClick = { userKeysSheet.open() }
        )
        Preference(
            title = { Text(text = stringResource(R.string.settings_pushover_url)) },
            summary = { Text(text = getSummary(state.serverData.settings.pushoverUrl)) },
            onClick = { urlSheet.open() }
        )
        PreferenceNote(stringResource(R.string.settings_pushover_note))
    }
    BottomSheet(
        sheetState = apiKeySheet.sheetState
    ) {
        InputValidationSheet(
            input = state.serverData.settings.pushoverApiKey,
            title = stringResource(R.string.settings_pushover_api),
            placeholder = stringResource(R.string.settings_pushover_api),
            onUpdate = {
                onEventSent(NotifContract.Event.SetPushOverAPIKey(it))
                apiKeySheet.close()
            },
            onDelete = {
                onEventSent(NotifContract.Event.SetPushOverAPIKey(""))
                apiKeySheet.close()
            }
        )
    }
    BottomSheet(
        sheetState = userKeysSheet.sheetState
    ) {
        InputValidationSheet(
            input = state.serverData.settings.pushoverUserKeys,
            title = stringResource(R.string.settings_pushover_keys),
            placeholder = stringResource(R.string.settings_pushover_keys),
            onUpdate = {
                onEventSent(NotifContract.Event.SetPushOverUserKeys(it))
                userKeysSheet.close()
            },
            onDelete = {
                onEventSent(NotifContract.Event.SetPushOverUserKeys(""))
                userKeysSheet.close()
            }
        )
    }
    BottomSheet(
        ...
        InputValidationSheet(
            input = state.serverData.settings.pushoverUrl,
            title = stringResource(R.string.settings_pushover_url),
            placeholder = stringResource(R.string.settings_pushover_url),
            onUpdate = {
                onEventSent(NotifContract.Event.SetPushOverUrl(it))
                urlSheet.close()
            },
            onDelete = {
                onEventSent(NotifContract.Event.SetPushOverUrl(""))
                urlSheet.close()
            }
        )
    }
...
private fun PushoverSettingsPreview() {
    PiFireTheme {
        ProvidePreferenceTheme {
            Surface {
                PushoverSettings(
