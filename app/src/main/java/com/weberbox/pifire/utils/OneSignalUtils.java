...
public class OneSignalUtils {
    ...
    public static void registerDevice(Context context, Socket socket,
                                      OneSignalResult registrationResult) {
    ...
    public static void checkOneSignalStatus(Activity activity, Socket socket) {
        OneSignalResult status = OneSignalUtils.checkRegistration(activity);
        switch (status) {
            case ONESIGNAL_NO_ID -> {
    ...
    public static OneSignalResult checkRegistration(Context context) {
        if (!OneSignal.userProvidedPrivacyConsent()) {
            return OneSignalResult.ONESIGNAL_NO_CONSENT;
        }

        OSDeviceState deviceState = OneSignal.getDeviceState();
        if (deviceState == null) {
            return OneSignalResult.ONESIGNAL_DEVICE_ERROR;
        } else if (deviceState.getPushToken() == null) {
            return OneSignalResult.ONESIGNAL_NULL_TOKEN;
        } else if (!deviceState.isSubscribed()) {
            return OneSignalResult.ONESIGNAL_NOT_SUBSCRIBED;
        }

        String playerID = deviceState.getUserId();
        if (playerID == null) {
            return OneSignalResult.ONESIGNAL_NO_ID;
        }

        Map<String, OneSignalDeviceInfo> devicesHash = getDevicesHash(context);

        if (devicesHash == null || !devicesHash.containsKey(playerID)) {
            return OneSignalResult.ONESIGNAL_NOT_REGISTERED;
        }
        ...
        for (Map.Entry<String, OneSignalDeviceInfo> device : devicesHash.entrySet()) {
            if (device.getKey().equals(playerID) &&
                ...
                return OneSignalResult.ONESIGNAL_APP_UPDATED;
            }
        }

        return OneSignalResult.ONESIGNAL_REGISTERED;
    }
